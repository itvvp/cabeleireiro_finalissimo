    ...
    if (! isset($error)) {
        ...
        // $data_hora_i=date('Y-m-d', strtotime($start))." ".date('H:i:s', strtotime($startTime));
            ...
            if ($numero_registos>0) {
                ...
                $sql="insert into events(title,start_event,end_event,color,text_color,id_tratamento,notas,cabeleireira) values('$title','$data_hora_i_c','$data_hora_f','#ff0000','#ffffff',9999,'$NotasHospede','2')";
                //echo $sql."<br>";
                $stmt = sqlsrv_query($conn, $sql);
                if ($stmt === false) {
                    die(print_r(sqlsrv_errors(), true));
