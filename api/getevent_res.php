...
if (isset($_REQUEST['id'])) {
    ...
    $sql="SELECT events.id, events.title, events.start_event, events.end_event, events.color, events.text_color, events.id_tratamento, events.nome_hospede, events.quarto, events.notas, events.cabeleireira, terapeutas.nome FROM events INNER JOIN terapeutas ON events.cabeleireira = terapeutas.id where events.id=$id";
    ...
    while($row = sqlsrv_fetch_array($stmt, SQLSRV_FETCH_ASSOC))
    {
        ...
        if (mb_detect_encoding($row['nome'], 'UTF-8', true)==false) {
            $cabeleireira=iconv('ISO-8859-1', 'UTF-8', $row["nome"]);
        } elseif (mb_detect_encoding($row['nome'])=='ASCII') {
            $cabeleireira=iconv('ISO-8859-1', 'UTF-8', $row["nome"]);
        } else {
            $cabeleireira= $row["nome"];
        } 
