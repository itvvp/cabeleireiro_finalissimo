...
if (isset($_POST['id'])) {
    ...
    if (! isset($error)) {
       ...
       // echo $id."<br><br>";
         ...
         $sql="select count(*) as total from events where (('$data_hora_i_c'>start_event and '$data_hora_i_c'<end_event) or ('$data_hora_f'>start_event and '$data_hora_f'<end_event)) and id_tratamento!=9999 and cabeleireira='1' and id!=$id";
     //   echo $sql."<br><br>";
        $stmt = sqlsrv_query($conn, $sql);
        if ($stmt === false) {
        ...
        if($perfil!=2)
        {
            ...
            ('$data_hora_f'>start_event and '$data_hora_f'<end_event)) and cabeleireira='1' and id_tratamento=9999";
            $stmt = sqlsrv_query($conn, $sql);
            if ($stmt === false) {
                die(print_r(sqlsrv_errors(), true));
            }
