...
//    $text_color = $_POST['text_color'];
   ...
   if ($id_tratamento == '') {
    ...
    if ($cabeleireira == '') {
        $error['cabeleireira'] = 'A seleção da cabeleireira é obrigatória';
    }
    if ($NomeHospede == '') {
...
//echo $perfil."<br>";
            ...
            if ($numero_registos>0) {
                ...
                if ($numero_impedimentos>0) {
                    ...
                    $sql="insert into events(title,start_event,end_event,color,text_color,id_tratamento,nome_hospede,quarto,notas,cabeleireira) values('$title','$data_hora_i_c','$data_hora_f','$cor','#ffffff',$id_tratamento,'$NomeHospede','$QuartoHospede','$NotasHospede','$cabeleireira')";
                    //echo $sql."<br>";
                    $stmt = sqlsrv_query($conn, $sql);
                    if ($stmt === false) {
                        die(print_r(sqlsrv_errors(), true));
